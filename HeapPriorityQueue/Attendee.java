...
public class Attendee implements Comparable<Attendee>{
	...
	public int compareTo(Attendee other) {
		if(eventTicket.compareTo(other.eventTicket) < 0){
			return -1;
		}else if(eventTicket.compareTo(other.eventTicket) > 0){
			return 1;
		}else{
			if(arrivalTime.compareTo(other.arrivalTime) < 0){
				return -1;
			}else if(arrivalTime.compareTo(other.arrivalTime) > 0){
				return 1;
			}else{
				return 0;
			}
